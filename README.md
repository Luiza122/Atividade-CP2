# 🧾 Atividade CP2 — API REST de Professores (Spring Boot + Oracle)

## 📘 Visão Geral

Este projeto implementa uma **API REST** em **Spring Boot 3.2.7** para o gerenciamento de professores, permitindo realizar operações CRUD (Criar, Ler, Atualizar e Deletar) armazenando os dados em um banco de dados **Oracle**.

A aplicação segue arquitetura em camadas (Controller → Service → Repository → Model) e inclui **documentação automática com Swagger UI**, além de uma **coleção pronta para o Insomnia**.

---

## 🧰 Tecnologias Utilizadas

- ☕ **Java 17**
- 🌱 **Spring Boot 3.2.7**
  - `spring-boot-starter-web`
  - `spring-boot-starter-jdbc`
- 🐘 **Oracle Database** + **JDBC Driver (ojdbc11 v23.5.0)**
- 📄 **Springdoc OpenAPI** (`springdoc-openapi-starter-webmvc-ui`) — para o Swagger UI
- 🧩 **Lombok**
- 🧪 **JUnit 5 + Mockito**
- 💾 **H2 Database** (para testes locais)
- 🧱 **Maven** (com suporte ao Maven Wrapper)

---

## ⚙️ Configuração do Ambiente

### 1️⃣ Pré-requisitos

- Java 17+
- Maven 3.8+
- Banco Oracle acessível
- IDE (ex: IntelliJ IDEA, Eclipse ou VS Code)

### 2️⃣ Criação da Tabela

Execute o script `script.sql` no seu banco Oracle:

```sql
DROP TABLE PROFESSOR;

CREATE TABLE PROFESSOR (
    ID            NUMBER(19) GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    NOME          VARCHAR2(150) NOT NULL,
    DEPARTAMENTO  VARCHAR2(150) NOT NULL,
    EMAIL         VARCHAR2(150) NOT NULL,
    TITULACAO     VARCHAR2(100) NOT NULL
);
```

---

## 🔐 Configuração de Conexão (Oracle)

Você pode definir as variáveis de ambiente antes de executar o projeto:

### Linux / macOS:
```bash
export ORACLE_URL="jdbc:oracle:thin:@oracle.fiap.com.br:1521:orcl"
export ORACLE_USER="seu_usuario"
export ORACLE_PASSWORD="sua_senha"
```

### Windows (CMD):
```cmd
set ORACLE_URL=jdbc:oracle:thin:@oracle.fiap.com.br:1521:orcl
set ORACLE_USER=seu_usuario
set ORACLE_PASSWORD=sua_senha
```

Essas variáveis são lidas pela classe `ConnectionFactory.java` (ou via `application.properties`).

---

## ▶️ Como Executar o Projeto

### Opção 1 — Usando Maven Wrapper
```bash
./mvnw spring-boot:run
```

### Opção 2 — Executar JAR
```bash
./mvnw clean package
java -jar target/restapi-0.0.1-SNAPSHOT.jar
```

### Opção 3 — IntelliJ IDEA
1. Abra o projeto (`File > Open > pasta do projeto`);
2. Espere o Maven baixar as dependências;
3. Configure variáveis de ambiente se necessário;
4. Rode a classe principal `RestapiApplication`.

A API ficará disponível em:
> **http://localhost:8081/api/professores**

---

## 📚 Endpoints da API

Base URL: `http://localhost:8081/api/professores`

| Método | Endpoint | Descrição |
|--------|-----------|------------|
| **GET** | `/` | Lista todos os professores |
| **GET** | `/{id}` | Busca um professor por ID |
| **POST** | `/` | Adiciona um novo professor |
| **PUT** | `/{id}` | Atualiza um professor existente |
| **DELETE** | `/{id}` | Remove um professor pelo ID |

---

### 🧾 Exemplos de Requisições

#### 🔹 Listar Professores
```bash
GET /api/professores
```
**Resposta:**
```json
[
  {
    "id": 1,
    "nome": "Fernando",
    "departamento": "Datascience",
    "email": "fernando@fiap.com",
    "titulacao": "Doutor"
  }
]
```

#### 🔹 Buscar por ID
```bash
GET /api/professores/1
```

#### 🔹 Adicionar Professor
```bash
POST /api/professores
Content-Type: application/json
```
**Body:**
```json
{
  "nome": "Fernando",
  "departamento": "Datascience",
  "email": "fernando@fiap.com",
  "titulacao": "Doutor"
}
```

#### 🔹 Atualizar Professor
```bash
PUT /api/professores/1
Content-Type: application/json
```
**Body:**
```json
{
  "nome": "Luiza",
  "departamento": "Back-End",
  "email": "luiza@example.com",
  "titulacao": "Professora"
}
```

#### 🔹 Deletar Professor
```bash
DELETE /api/professores/1
```

📦 **Coleção Insomnia:**  
O arquivo [`Collection_Professor.json`](Collection_Professor.json) contém todas as requisições configuradas (GET, POST, PUT, DELETE).  
Basta importar no **Insomnia** para testar diretamente.

---

## 🧱 Estrutura do Projeto

```
Atividade-CP2/
├── pom.xml
├── script.sql
├── Collection_Professor.json
├── src/
│   ├── main/java/com/fiap/restapi/
│   │   ├── RestapiApplication.java        # Classe principal
│   │   ├── controller/ProfessorController.java
│   │   ├── service/ProfessorService.java
│   │   ├── repository/ProfessorRepository.java
│   │   ├── model/Professor.java
│   │   └── config/ConnectionFactory.java
│   └── resources/
│       └── application.properties
└── target/
```

---

## 🌐 Swagger UI

Documentação automática da API:
> **http://localhost:8081/swagger-ui**

Especificação OpenAPI JSON:
> **http://localhost:8081/v3/api-docs**

---

## ⚠️ Possíveis Erros e Soluções

| Erro | Causa Provável | Solução |
|------|----------------|----------|
| `ORA-00942: table or view does not exist` | Tabela PROFESSOR não criada | Execute o `script.sql` |
| `ORA-01017: invalid username/password` | Credenciais incorretas | Corrija as variáveis de ambiente |
| `Port 8081 already in use` | Outra aplicação usa a porta | Altere `server.port` no `application.properties` |
| `500 Internal Server Error` | Falha de validação ou SQL | Verifique os campos JSON e logs no console |

---

## 📄 Licença

Este projeto foi desenvolvido para fins **acadêmicos (FIAP)**.  
Sinta-se à vontade para estudar, testar e modificar o código.

**© 2025 — FIAP | Engenharia de Software**
